...
export async function getProfile(): Promise<UserProfile | null> {
  return withPerformanceTracking('getProfile', async () => {
...
export async function getCadetDashboardData(userId: string): Promise<CadetDashboardData> {
  return withPerformanceTracking('getCadetDashboardData', async () => {
    const supabase = await createClient();
    ...
    try {
      ...
      if (eventsError) Logger.error('Error fetching events:', eventsError);
      ...
      if (attendanceError) Logger.error('Error fetching attendance logs:', attendanceError);
      ...
      if (announcementsError) Logger.error('Error fetching announcements:', announcementsError);
      ...
      Logger.error('Unexpected error in getCadetDashboardData', error);
      return {
        nextEvent: null,
        attendanceScore: 100,
        recentAnnouncements: [],
...
export async function getOfficerDashboardData(): Promise<OfficerDashboardData> {
  return withPerformanceTracking('getOfficerDashboardData', async () => {
    const supabase = await createClient();
    ...
    try {
      ...
      if (strengthError) Logger.error('Error fetching unit strength:', strengthError);
      ...
      if (attendanceError) Logger.error('Error fetching all attendance:', attendanceError);
      ...
      Logger.error('Unexpected error in getOfficerDashboardData', error);
      return {
        unitStrength: 0,
